 ...
 class TPAlgTPCAbsRunningSum : public TPAlgTPCTool {
  ...
  public:
    explicit TPAlgTPCAbsRunningSum(fhicl::ParameterSet const& ps) :
      ...
      scale_factor_(ps.get<int>("scale_factor",2)), //AbsRS-specific parameter ensuring the scale of hit parameters remains in the same ballpark as for simple threshold alg
   ...
   {
     //R-value should be a float in range [0,1], but we're working with integers online. So multiply everything by 10 and then de-scale. 
			  ...
			  std::vector<dunedaq::trgdataformats::TriggerPrimitive> & tps_out) 
    {
	  ...
	  if (threshold_ < 0) continue; 
	    ...
	    //	    if (channel == 10236)  std::cout << sample << "," << std::endl; 
		...
		if (channel == 10236)  std::cout << this_tp.time_peak << "," <<  this_tp.adc_peak << "," << this_tp.time_over_threshold << std::endl; 
